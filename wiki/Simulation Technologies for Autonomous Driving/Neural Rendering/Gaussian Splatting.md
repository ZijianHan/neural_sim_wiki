...
3. **Automotive multimodal route**: SplatAD (camera + lidar rendering), Flow4DGS-SLAM (dynamic 4DGS + SLAM), DGGT (pose-free feedforward dynamic 4D reconstruction).
...
3. **自动驾驶多模态路线**：SplatAD（相机+雷达）、Flow4DGS-SLAM（动态 4DGS + SLAM）与 DGGT（无位姿前馈动态 4D 重建）。
